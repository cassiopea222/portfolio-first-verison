/**
 * Verbatim copy of pretext's pages/demos/wrap-geometry.ts
 * Source: https://github.com/chenglou/pretext (MIT, © 2024 chenglou)
 *
 * Provides:
 *   - getWrapHull(src, options): rasterize an image, derive a normalized 0..1 silhouette polygon
 *   - transformWrapPoints(points, rect, angle): place hull points into screen coords
 *   - getPolygonIntervalForBand(...) / getRectIntervalsForBand(...)
 *   - carveTextLineSlots(base, blocked): subtract blocked intervals from one row
 *   - isPointInPolygon(...) for hit testing
 */
 ...
 *
 * Discards absurdly narrow leftover slivers that we would never want to
 * hand to text layout.
 */
...
async function makeWrapHull(src: string, options: WrapHullOptions): Promise<Point[]> {
  ...
  const width =
    aspect >= 1 ? maxDimension : Math.max(64, Math.round(maxDimension * aspect));
  const height =
    aspect >= 1 ? Math.max(64, Math.round(maxDimension / aspect)) : maxDimension;
